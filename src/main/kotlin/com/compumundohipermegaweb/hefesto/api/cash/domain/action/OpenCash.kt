    ...
    private fun openCash(openRequest: OpenRequest): Cash? {
        ...
        if (cash != null) {
            if(cash.status == "CLOSE") {
                ...
                cashMovementRepository.save(CashMovement(0L, cashStartEnd.id, "INGRESO", Date(), 0L, "APERTURA", 0L, 0L, openRequest.userId, openRequest.openingBalance, "APERTURA DE CAJA"), cashStartEnd.id)
            }
        }
        return cash
    }
